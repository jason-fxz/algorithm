...
struct node {
    datmn mn; datmx mx; // mn: (最小值，严格次小值，最小值个数) , mx: (最大值，严格次大值，最大值个数)
    ll sum; int addmn, addmx, add, len; // sum 区间和，addmn 最小值lazytag，addmx 最大值lazytag，add 其他数 lazytag，len 区间长度 
}t[N << 2];
...
void update(int x, int vn, int vx, int v) { // vn: addmn, vx: addmx, v: add
    if (t[x].mn.fi == t[x].mx.fi) { // 所有数相同特判，此时最大值 tag 和最小值 tag 应该相同且不等于其他值 tag
        if (vn == v) vn = vx; else vx = vn;
